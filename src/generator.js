  ...
  const generators = {
    ...
    VariableDeclaration(d) {
      ...
      if (d.initializer.kind === "FunctionEvoke") {
        state.output.push(
          `${keyword} ${gen(d.variable)} = ${gen(d.initializer, "exp")};`
        );
        return;
      }
      state.output.push(
        `${keyword} ${gen(d.variable)} = ${gen(d.initializer)};`
      );
    },
    Variable(v) {
      return targetName(v);
    ...
    FunctionEvoke(decl, context) {
      let saved_state_output;
      if (context === "exp") {
        saved_state_output = state.output;
        state.output = [];
      }
      state.output.push(
        `function ${gen(decl.fun)}(${decl.fun.parameters
          .map(gen)
          .join(", ")}) {`
      );
      ...
      state.output.push("}");
      if (context === "exp") {
        const generated = state.output.join("\n");
        state.output = saved_state_output;
        return generated;
      }
    },
    Function(f) {
    ...
    ExscribeStatement(e) {
        console.log("exscribe statement", e.expression);
